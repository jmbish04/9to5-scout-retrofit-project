...
class TalentAPITester:
    ...
    def _test_worker_talent_search(self, query: str = "software engineer") -> TestResult:
        """Test job search through Worker API"""
        start_time = time.time()
        try:
            headers = {
            ...
            response = self._make_request(
                'POST', 
                f"{self.worker_url}/api/talent/search", 
                headers=headers,
                json=payload
            )
    ...
    def _test_worker_talent_suggestions(self, query: str = "software") -> TestResult:
        """Test job suggestions through Worker API"""
        start_time = time.time()
        try:
            headers = {
                'Authorization': f'Bearer {self.api_key}',
            ...
            response = self._make_request(
                'GET', 
                f"{self.worker_url}/api/talent/suggestions", 
                headers=headers,
                params=params
            )
    ...
    def _print_result(self, result: TestResult) -> None:
        ...
        if result.data and result.success:
            # Print relevant data
            if 'matchingJobs' in result.data:
