...
export class ContentUtils {
     ...
     */
    static async fetchRenderedText(env, targetUrl) {
     ...
     */
    static async urlToMarkdown(env, targetUrl) {
        ...
        const result = await this.getLLMResult(env, `${prompt}\n\n---\n\n${textContent.substring(0, 15000)}`, { schema });
        return result.markdown ?? "";
    }
    /**
     * Generate a PDF from HTML and optional CSS.
     */
    static async generatePdf(env, html, css) {
        const browser = await puppeteer.launch(env.MYBROWSER);
        try {
            const page = await browser.newPage();
...
    `;
    ...
    static async getLLMResult(env, prompt, jsonSchema = DEFAULT_RESPONSE_SCHEMA) {
        ...
        try {
            ...
            const raw = aiResult?.response ?? aiResult;
            ...
            const cause = err && typeof err === "object" && "cause" in err
                ...
                : undefined;
            throw new Error(`AI.run failed for model "${model}": ${message}${cause ? ` | cause: ${cause}` : ""}`);
        }
    }
}
