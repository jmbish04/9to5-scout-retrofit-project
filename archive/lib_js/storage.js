...
export async function saveJob(env, job) {
    ...
    let id = job.id;
    let existingFirstSeenAt;
    if (!id && job.url) {
        const existingJob = await env.DB.prepare("SELECT id, first_seen_at FROM jobs WHERE url = ?")
            .bind(job.url)
            .first();
        if (existingJob) {
            id = existingJob.id;
    ...
    await env.DB.prepare(`INSERT OR REPLACE INTO jobs(
      ...
      daily_monitoring_enabled, monitoring_frequency_hours, last_status_check_at, closure_detected_at
        ...
        .bind(id, job.site_id, job.url, job.canonical_url, job.title, job.company, job.location, job.employment_type, job.department, job.salary_min, job.salary_max, job.salary_currency, job.salary_raw, job.compensation_raw, job.description_md, job.requirements_md, job.posted_at, job.closed_at, job.status || "open", job.source || "SCRAPED", job.last_seen_open_at, job.first_seen_at || existingFirstSeenAt || new Date().toISOString(), new Date().toISOString(), job.daily_monitoring_enabled !== undefined
        ? job.daily_monitoring_enabled
        : true, job.monitoring_frequency_hours || 24, job.last_status_check_at, job.closure_detected_at)
        .run();
 ...
 */
export async function getJobs(env, options = {}) {
...
export async function saveSite(env, site) {
    ...
    await env.DB.prepare(`INSERT OR REPLACE INTO sites(
      id, name, base_url, robots_txt, sitemap_url, 
      discovery_strategy, last_discovered_at, created_at
    ) VALUES(?1,?2,?3,?4,?5,?6,?7,?8)`)
        .bind(id, site.name, site.base_url, site.robots_txt, site.sitemap_url, site.discovery_strategy, site.last_discovered_at, site.created_at || new Date().toISOString())
        .run();
    return id;
}
/**
 ...
 */
export async function saveSearchConfig(env, config) {
    const id = config.id || crypto.randomUUID();
    await env.DB.prepare(`INSERT OR REPLACE INTO search_configs(
      id, name, keywords, locations, include_domains, 
      exclude_domains, min_comp_total, created_at, updated_at
    ) VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)`)
        .bind(id, config.name, config.keywords, config.locations, config.include_domains, config.exclude_domains, config.min_comp_total, config.created_at || new Date().toISOString(), new Date().toISOString())
        .run();
    return id;
}
/**
 ...
 */
export async function getSearchConfigs(env) {
 ...
 */
export async function createRun(env, type, config_id) {
    const id = crypto.randomUUID();
    await env.DB.prepare("INSERT INTO runs(id, type, config_id, status) VALUES(?1, ?2, ?3, ?4)")
 ...
 * Get recent runs.
 */
export async function getRuns(env, limit = 20) {
    const result = await env.DB.prepare("SELECT * FROM runs ORDER BY started_at DESC LIMIT ?1")
        .bind(limit)
        .all();
    return result.results || [];
}
 ...
 */
export async function createSnapshot(env, snapshot) {
    const id = crypto.randomUUID();
    ...
    await env.DB.prepare(`INSERT INTO snapshots(
      ...
      screenshot_r2_key, pdf_r2_key, markdown_r2_key, fetched_at, http_status, etag
        ...
        .bind(id, snapshot.job_id, snapshot.run_id, snapshot.content_hash, r2Keys.html_r2_key, r2Keys.json_r2_key, r2Keys.screenshot_r2_key, r2Keys.pdf_r2_key, r2Keys.markdown_r2_key, timestamp, snapshot.http_status, snapshot.etag)
        .run();
    return id;
}
/**
 ...
 */
export async function getJobsForMonitoring(env) {
 ...
 */
export async function updateJobStatus(env, jobId, status, closureDetected) {
    const now = new Date().toISOString();
    if (closureDetected) {
        await env.DB.prepare("UPDATE jobs SET status = ?, last_status_check_at = ?, closure_detected_at = ? WHERE id = ?")
            .bind(status, now, now, jobId)
            .run();
    }
    else {
        await env.DB.prepare("UPDATE jobs SET status = ?, last_status_check_at = ? WHERE id = ?")
 ...
 */
export async function createJobTrackingHistory(env, entry) {
    const id = crypto.randomUUID();
    await env.DB.prepare(`INSERT INTO job_tracking_history(
      ...
      title_changed, requirements_changed, salary_changed, description_changed, error_message
        ...
        .bind(id, entry.job_id, entry.snapshot_id, entry.tracking_date, entry.status, entry.content_hash, entry.title_changed || false, entry.requirements_changed || false, entry.salary_changed || false, entry.description_changed || false, entry.error_message)
        .run();
    return id;
}
/**
 ...
 */
export async function getJobTrackingHistory(env, jobId, limit = 30) {
    const result = await env.DB.prepare(`
    SELECT 
      jth.*,
      s.html_r2_key,
 ...
 */
export async function saveJobMarketStats(env, date, stats) {
    await env.DB.prepare(`INSERT OR REPLACE INTO job_market_stats(
      ...
      avg_job_duration_days, top_companies, trending_keywords, salary_stats, location_stats
        ...
        .bind(crypto.randomUUID(), date, stats.total_jobs_tracked, stats.new_jobs_found, stats.jobs_closed, stats.jobs_modified, stats.avg_job_duration_days, stats.top_companies ? JSON.stringify(stats.top_companies) : null, stats.trending_keywords ? JSON.stringify(stats.trending_keywords) : null, stats.salary_stats ? JSON.stringify(stats.salary_stats) : null, stats.location_stats ? JSON.stringify(stats.location_stats) : null)
        .run();
}
 ...
 */
export async function getApplicantProfile(env, userId) {
    const result = await env.DB.prepare("SELECT * FROM applicant_profiles WHERE user_id = ?")
        .bind(userId)
        .first();
    if (!result)
        return null;
 ...
 */
export async function createApplicantProfile(env, userId) {
    const id = crypto.randomUUID();
    const profile = {
        id,
        user_id: userId,
 ...
 */
export async function updateApplicantProfile(env, applicantId, updates) {
    const setClause = [];
    const values = [];
 ...
 */
export async function saveJobHistorySubmission(env, submission) {
    await env.DB.prepare(`
    INSERT INTO job_history_submissions 
    (id, applicant_id, raw_content, content_type, processing_status, submitted_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `)
        .bind(submission.id, submission.applicant_id, submission.raw_content, submission.content_type, submission.processing_status, submission.submitted_at)
        .run();
}
/**
 * Update a job history submission.
 */
export async function updateJobHistorySubmission(env, submissionId, updates) {
    const setClause = [];
    const values = [];
    for (const [key, value] of Object.entries(updates)) {
 ...
 */
export async function saveJobHistoryEntry(env, entry) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
        .bind(id, entry.applicant_id, entry.company_name, entry.job_title, entry.department, entry.employment_type, entry.start_date, entry.end_date, entry.is_current ? 1 : 0, entry.location, entry.salary_min, entry.salary_max, entry.salary_currency, entry.responsibilities, entry.achievements, entry.skills_used ? JSON.stringify(entry.skills_used) : null, entry.technologies ? JSON.stringify(entry.technologies) : null, entry.keywords ? JSON.stringify(entry.keywords) : null, now, now)
        .run();
 ...
 */
export async function getJobHistoryByApplicant(env, applicantId) {
    const results = await env.DB.prepare("SELECT * FROM job_history WHERE applicant_id = ? ORDER BY start_date DESC")
        .bind(applicantId)
        .all();
 ...
 */
export async function getJobHistorySubmissions(env, applicantId) {
    const results = await env.DB.prepare("SELECT * FROM job_history_submissions WHERE applicant_id = ? ORDER BY submitted_at DESC")
        .bind(applicantId)
        .all();
 ...
 */
export async function saveJobRating(env, rating) {
  ...
  `)
        .bind(id, rating.applicant_id, rating.job_id, rating.overall_score, rating.skill_match_score, rating.experience_match_score, rating.compensation_fit_score, rating.location_fit_score, rating.company_culture_score, rating.growth_potential_score, rating.rating_summary, rating.recommendation, rating.strengths ? JSON.stringify(rating.strengths) : null, rating.gaps ? JSON.stringify(rating.gaps) : null, rating.improvement_suggestions, now, now)
        .run();
 ...
 */
export async function getJobRatingsByApplicant(env, applicantId) {
