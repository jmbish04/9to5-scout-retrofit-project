 ...
 */
export function extractJobUrls(content) {
    const urls = [];
 ...
 */
export async function extractJobInfo(env, content) {
    try {
        ...
        if (responseData?.response) {
            const jobData = typeof responseData.response === "string"
 ...
 */
export async function parseEmailFromRequest(request) {
    try {
 ...
 */
export async function generateEmailInsights(env, config) {
 ...
 */
export async function sendInsightsEmail(insights, config, env) {
    try {
        ...
        const message = new CloudflareEmailMessage(`digest@${env.EMAIL_ROUTING_DOMAIN}`, // From
 ...
 */
export function formatInsightsEmail(insights, periodHours) {
        ...
        <table class="stats-table">
            ...
            .map((stat) => `
                <tr>
                    <td>${stat.role}</td>
                    <td>${stat.count}</td>
                    <td>${stat.avgMinSalary
            ? `$${stat.avgMinSalary.toLocaleString()}`
            : "N/A"}</td>
                    <td>${stat.avgMaxSalary
