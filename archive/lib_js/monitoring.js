...
import { createJobTrackingHistory, getJobsForMonitoring, saveJobMarketStats, updateJobStatus, } from "./storage";
/**
 * Run daily monitoring for all active jobs.
 * This function should be called by the scheduled worker.
 */
export async function runDailyJobMonitoring(env) {
        ...
        for (let i = 0; i < jobsToMonitor.length; i += batchSize) {
            ...
            for (const promiseResult of batchResults) {
                if (promiseResult.status === "fulfilled") {
                    ...
                    if (jobResult.modified)
                        result.jobs_modified++;
                    if (jobResult.closed)
                        result.jobs_closed++;
                    if (jobResult.snapshotCreated)
                        result.snapshots_created++;
                    if (jobResult.pdfGenerated)
                        result.pdfs_generated++;
                    if (jobResult.markdownExtracted)
 ...
 */
async function monitorSingleJob(env, job, trackingDate) {
    const jobResult = {
        modified: false,
        closed: false,
    ...
    `)
      ...
      `)
        ...
        await createJobTrackingHistory(env, {
            ...
            content_hash: (await env.DB.prepare("SELECT content_hash FROM snapshots WHERE id = ?")
                .bind(snapshotId)
                .first())?.content_hash,
 ...
 */
async function generateDailyMarketStats(env, date, monitoringResult) {
    try {
    ...
    `)
        ...
        const keywordCounts = {};
        trendingKeywords.results?.forEach((job) => {
    ...
    `).all();
        ...
        await saveJobMarketStats(env, date, {
            ...
            avg_job_duration_days: avgDuration?.avg_days || undefined,
 ...
 */
export async function getJobTrackingTimeline(env, jobId) {
    try {
        // Get job details
