 ...
 */
export async function handleAgentsGet(request, env) {
    try {
...
export async function handleAgentsPost(request, env) {
    try {
    ...
    `)
            .bind(agent.id, agent.name, agent.role, agent.goal, agent.backstory, agent.llm, agent.system_prompt || null, agent.max_tokens || 4000, agent.temperature || 0.7, agent.enabled !== false ? 1 : 0, now, now)
            .run();
        const createdAgent = {
            ...agent,
...
export async function handleAgentGet(request, env, agentId) {
    try {
        const { results } = await env.DB.prepare("SELECT * FROM agent_configs WHERE id = ?")
            .bind(agentId)
            .all();
        if (results.length === 0) {
...
export async function handleAgentPut(request, env, agentId) {
    try {
        const agent = await request.json();
        const now = new Date().toISOString();
        // Check if agent exists
        const { results } = await env.DB.prepare("SELECT id FROM agent_configs WHERE id = ?")
            .bind(agentId)
            .all();
        if (results.length === 0) {
            return new Response(JSON.stringify({ error: "Agent not found" }), {
      ...
      WHERE id = ?
    `)
            .bind(agent.name || null, agent.role || null, agent.goal || null, agent.backstory || null, agent.llm || null, agent.system_prompt || null, agent.max_tokens || null, agent.temperature || null, agent.enabled !== undefined ? (agent.enabled ? 1 : 0) : null, now, agentId)
            .run();
        // Fetch updated agent
        const { results: updatedResults } = await env.DB.prepare("SELECT * FROM agent_configs WHERE id = ?")
            .bind(agentId)
            .all();
...
export async function handleAgentDelete(request, env, agentId) {
    try {
        ...
        const { results: taskResults } = await env.DB.prepare("SELECT COUNT(*) as count FROM task_configs WHERE agent_id = ?")
            .bind(agentId)
            .all();
        const taskCountResult = taskResults[0];
