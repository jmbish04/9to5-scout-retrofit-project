 ...
 */
import { createApplicantProfile, getApplicantProfile, getJobById, getJobHistoryByApplicant, getJobHistorySubmissions, getJobRatingsByApplicant, saveJobHistoryEntry, saveJobHistorySubmission, saveJobRating, updateApplicantProfile, updateJobHistorySubmission, } from "../lib/storage";
export async function handleJobHistoryPost(request, env) {
    try {
...
export async function handleJobHistoryGet(request, env, params) {
    try {
...
export async function handleJobRatingPost(request, env) {
    try {
        const body = (await request.json());
...
export async function handleJobRatingsGet(request, env, params) {
    try {
        const { user_id } = params;
        if (!user_id) {
            return new Response(JSON.stringify({ error: "Missing user_id parameter" }), {
...
async function processJobHistoryWithAI(env, rawContent, applicant) {
    const prompt = `
...
async function generateJobRating(env, applicant, jobHistory, job) {
    const prompt = `
