 ...
 */
export async function handleScrapeQueuePost(request, env) {
    try {
        ...
        const result = await env.DB.prepare(`INSERT INTO scrape_queue (urls, source, status, priority, payload, available_at, created_at, updated_at)
       VALUES (?, ?, 'pending', ?, ?, ?, ?, ?)`)
            .bind(body.urls, body.source || null, body.priority || 0, body.payload || null, now, now, now)
            .run();
        const job = {
        ...
        return new Response(JSON.stringify({ error: "Failed to add job to queue" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        });
    }
}
export async function handleScrapeQueuePendingGet(request, env) {
    try {
        ...
        const result = await env.DB.prepare(`SELECT * FROM scrape_queue 
       ...
       LIMIT ?`)
        ...
        const now = new Date().toISOString();
        await env.DB.prepare(`UPDATE scrape_queue 
       ...
       WHERE id IN (${placeholders}) AND status = 'pending'`)
        ...
        return new Response(JSON.stringify({ error: "Failed to fetch pending jobs" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        });
    }
}
export async function handleScrapeQueuePatch(request, env, jobId) {
    try {
        const body = (await request.json());
        ...
        const result = await env.DB.prepare(`UPDATE scrape_queue 
       SET ${updateFields.join(", ")} 
       WHERE id = ? AND status = 'claimed'`)
            .bind(...bindValues)
            .run();
        if (result.meta.changes === 0) {
            return new Response(JSON.stringify({ error: "Job not found or not in claimed status" }), {
                status: 404,
                headers: { "Content-Type": "application/json" },
            });
        }
