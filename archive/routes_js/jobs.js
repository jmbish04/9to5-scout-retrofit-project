...
export async function handleJobsGet(request, env) {
    try {
...
export async function handleJobGet(request, env, jobId) {
    try {
...
export async function handleJobsBatchPost(request, env) {
    try {
        ...
        return new Response(JSON.stringify({ error: "Failed to process batch jobs" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        });
    }
