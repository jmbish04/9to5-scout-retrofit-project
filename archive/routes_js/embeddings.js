...
export async function handleCreateEmbedding(request, env) {
    try {
        ...
        if (!validContentTypes.includes(contentType)) {
            return new Response(JSON.stringify({
                error: "Invalid contentType. Must be one of: " +
...
export async function handleUpdateEmbedding(request, env, uuid) {
    try {
        const body = (await request.json());
...
export async function handleDeleteEmbedding(request, env, uuid) {
    try {
        const url = new URL(request.url);
        const contentType = url.searchParams.get("contentType");
        if (!contentType) {
            return new Response(JSON.stringify({ error: "contentType query parameter is required" }), {
                status: 400,
                headers: { "Content-Type": "application/json" },
            });
        }
        const embeddingsManager = new EmbeddingsManager(env);
        const result = await embeddingsManager.deleteAssetEmbedding(uuid, contentType);
        if (result.success) {
            return new Response(JSON.stringify({
                success: true,
                data: result,
...
export async function handleSearchEmbeddings(request, env) {
    try {
        const url = new URL(request.url);
...
export async function handleSearchByUUID(request, env, uuid) {
    try {
        const url = new URL(request.url);
        const limit = url.searchParams.get("limit");
        const embeddingsManager = new EmbeddingsManager(env);
        const results = await embeddingsManager.searchByUUID(uuid, limit ? parseInt(limit) : 10);
        return new Response(JSON.stringify({
            success: true,
            data: results,
...
export async function handleGetEmbeddingStats(request, env) {
    try {
        const url = new URL(request.url);
        const contentType = url.searchParams.get("contentType");
        let query = `
...
export async function handleGetEmbeddingOperations(request, env) {
    try {
        const url = new URL(request.url);
