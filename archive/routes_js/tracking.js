 ...
 */
export async function handleJobTrackingGet(request, env) {
    try {
 ...
 */
export async function handleSnapshotContentGet(request, env) {
    try {
        const url = new URL(request.url);
        const pathParts = url.pathname.split("/");
    ...
    `)
        ...
        switch (contentType) {
            case "html":
                r2Key = snapshotData.html_r2_key || null;
                mimeType = "text/html";
                break;
            case "pdf":
                r2Key = snapshotData.pdf_r2_key || null;
                mimeType = "application/pdf";
                break;
            case "markdown":
                r2Key = snapshotData.markdown_r2_key || null;
                mimeType = "text/markdown";
                break;
            case "json":
                r2Key = snapshotData.json_r2_key || null;
                mimeType = "application/json";
                break;
            case "screenshot":
                r2Key = snapshotData.screenshot_r2_key || null;
                mimeType = "image/png";
                break;
            default:
 ...
 */
export async function handleDailyMonitoringPost(request, env) {
    try {
 ...
 */
export async function handleMonitoringStatusGet(request, env) {
    try {
 ...
 */
export async function handleMonitoringQueueGet(request, env) {
    try {
        const url = new URL(request.url);
 ...
 */
export async function handleJobMonitoringPut(request, env) {
    try {
        const url = new URL(request.url);
        const pathParts = url.pathname.split("/");
