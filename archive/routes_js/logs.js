...
logs.post("/test", async (c) => {
    ...
    try {
        ...
        const { session_id, test_name, success, duration, error, data, timestamp, test_type, } = body;
    ...
    `)
            .bind(session_id, test_name, success, duration, error || null, data || null, timestamp, test_type)
            .run();
