...
import { cors, errorHandler, logger, rateLimit, validateBody, validateParams, } from "../lib/hono-validation";
import { JobSite, createJobScraper } from "../lib/steel";
import { BulkScrapeRequestSchema, JobSiteSchema, ScrapeJobRequestSchema, ScrapeJobsRequestSchema, SearchJobsRequestSchema, } from "../lib/validation";
const app = new Hono();
...
app.get("/sites", async (c) => {
    try {
        ...
        const dbSites = await c.env.DB.prepare("SELECT * FROM sites ORDER BY name").all();
        ...
        const response = {
            sites: dbSites.results.map((site) => {
                ...
                const steelConfig = scraper.getSiteConfig(site.id);
                ...
                return {
                    ...
                    isSteelSupported: steelSites.includes(site.id),
 ...
 * }
 */
app.post("/search/:site", validateParams(z.object({ site: JobSiteSchema })), validateBody(SearchJobsRequestSchema), async (c) => {
    ...
    try {
        ...
        const siteRecord = await c.env.DB.prepare("SELECT * FROM sites WHERE id = ?")
            .bind(site)
            .first();
 ...
 * }
 */
app.post("/scrape/:site", validateParams(z.object({ site: JobSiteSchema })), validateBody(ScrapeJobsRequestSchema), async (c) => {
    // Get validated data from context - TypeScript safe access
    const validatedParams = c.get("validatedParams");
    const validatedBody = c.get("validatedBody");
    const { site } = validatedParams;
    ...
    try {
        ...
        // Scrape jobs
        const results = await scraper.scrapeJobsBatch(site, jobUrls, c.env, batchSize);
 ...
 * }
 */
app.post("/scrape-job/:site", validateParams(z.object({ site: JobSiteSchema })), validateBody(ScrapeJobRequestSchema), async (c) => {
    // Get validated data from context - TypeScript safe access
    const validatedParams = c.get("validatedParams");
    const validatedBody = c.get("validatedBody");
    const { site } = validatedParams;
 ...
 * }
 */
app.post("/bulk-scrape", validateBody(BulkScrapeRequestSchema), async (c) => {
    // Get validated data from context - TypeScript safe access
    const validatedBody = c.get("validatedBody");
    ...
    const siteRecords = await c.env.DB.prepare(`SELECT id FROM sites WHERE id IN (${siteIds.map(() => "?").join(",")})`)
        .bind(...siteIds)
        .all();
    const existingSiteIds = new Set(siteRecords.results.map((r) => r.id));
    ...
    for (const siteConfig of sites) {
        ...
        try {
            ...
            // Scrape jobs
            const scrapeResults = await scraper.scrapeJobsBatch(site, jobUrls, c.env, batchSize);
            // Store results in D1 database
            const storedJobs = [];
            for (const result of scrapeResults) {
                try {
                    ...
                    console.error(`Failed to store job ${result.job.id} in database:`, dbError);
    ...
    const totalScraped = results.reduce((sum, result) => sum + (result.totalScraped || 0), 0);
    const response = {
        summary: {
