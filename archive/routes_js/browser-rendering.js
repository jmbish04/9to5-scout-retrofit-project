...
import { createBrowserRenderingClient, } from "../lib/browser-rendering";
import { cors, errorHandler, logger, rateLimit, validateBody, } from "../lib/hono-validation";
import { ComprehensiveScrapeRequestSchema, ContentRequestSchema, JsonRequestSchema, LinksRequestSchema, MarkdownRequestSchema, PdfRequestSchema, ScrapeRequestSchema, ScreenshotRequestSchema, SnapshotRequestSchema, } from "../lib/validation";
const app = new Hono();
...
app.post("/content", validateBody(ContentRequestSchema), async (c) => {
    try {
        ...
        const { options } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        const content = await client.getContent(options);
        return c.json({
            success: true,
            content,
...
app.post("/pdf", validateBody(PdfRequestSchema), async (c) => {
    try {
        ...
        const { options } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        const pdf = await client.generatePdf(options);
        // Convert ArrayBuffer to base64 for JSON response
...
app.post("/markdown", validateBody(MarkdownRequestSchema), async (c) => {
    try {
        ...
        const { options } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        const markdown = await client.extractMarkdown(options);
        return c.json({
            success: true,
            markdown,
...
app.post("/links", validateBody(LinksRequestSchema), async (c) => {
    try {
        ...
        const { options } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        const links = await client.getLinks(options);
        return c.json({
            success: true,
            links,
            count: links.length,
...
app.post("/snapshot", validateBody(SnapshotRequestSchema), async (c) => {
    try {
        ...
        const { options } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        const snapshot = await client.takeSnapshot(options);
        return c.json({
            success: true,
            snapshot: {
 ...
 */
app.post("/comprehensive", validateBody(ComprehensiveScrapeRequestSchema), async (c) => {
    try {
        const validatedBody = c.get("validatedBody");
        const { url, includeHtml, includeScreenshot, includePdf, includeMarkdown, includeJson, includeLinks, includeSnapshot, includeScraped, scrapeElements, jsonPrompt, jsonSchema, authentication, viewport, screenshotOptions, gotoOptions, jobId, siteId, } = validatedBody;
        const client = createBrowserRenderingClient(c.env);
        ...
        if (jobId) {
            const storeResult = await client.storeResult(c.env.DB, result, jobId, siteId);
            if (!storeResult.success) {
                console.warn("Failed to store result in database:", storeResult.error);
            }
        }
        return c.json({
            success: true,
