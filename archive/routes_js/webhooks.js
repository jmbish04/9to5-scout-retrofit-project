 ...
 */
export async function notifyNewJobs(env, jobs, siteName) {
    ...
    try {
        const jobSummary = jobs
            .slice(0, 3)
            .map((job) => `• *${job.title}* at ${job.company} ${job.location ? `(${job.location})` : ""}`)
            .join("\n");
 ...
 */
export async function notifyJobStatusChange(env, job, oldStatus, newStatus) {
    if (!env.SLACK_WEBHOOK_URL) {
        return;
    }
    try {
