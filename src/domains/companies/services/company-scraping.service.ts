...
const ROBOTS_CACHE = new Map<
  string,
  { fetchedAt: number; disallows: string[] }
>();
...
async function fetchWithRetries(
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, delayBase * (attempt + 1) + jitter)
    );
  }
...
async function isAllowed(url: string): Promise<boolean> {
  try {
    ...
    return !disallows.some(
      (rule) => rule !== "" && parsed.pathname.startsWith(rule)
    );
  } catch (error) {
  ...
  while ((match = anchorPattern.exec(html)) !== null) {
    ...
    if (href && /(benefit|perk|total\s+rewards)/i.test(normalized)) {
      try {
        ...
        console.warn(
          "Skipping invalid href while extracting benefits links",
          href,
          error
        );
      }
...
async function storeSnapshot(
  ...
  if (existing) {
    console.log(
      "Skipping duplicate benefits snapshot for company",
      companyId,
      sourceUrl
    );
    return;
  ...
  if (dryRun) {
    console.log(
      "[DRY_RUN] Would insert benefits snapshot for company",
      companyId,
      sourceUrl
    );
    return;
...
async function scrapeSingleUrl(
  ...
  const requestInit: RequestInit = {
    ...
    body: JSON.stringify({
      ...
      context: {
        userAgent:
          "Mozilla/5.0 (compatible; 9to5-scout/1.0; +https://9to5scout.com)",
      },
    }),
  };
...
async function scrapeAndStoreBenefits(
  env: CompanyScrapeEnv,
  company: CompanyRow,
  url: string,
  ...
  try {
    ...
    for (const link of additionalLinks) {
      try {
        ...
        const { snapshotText: subSnapshot, parsed: subParsed } =
          await extractBenefits(subText || subHtml);
    ...
    console.error(
      "Failed to scrape benefits for company",
      company.id,
      url,
      error
    );
...
export async function companyBenefitsNightly(
  env: CompanyScrapeEnv,
  options: ScrapeOptions = {}
): Promise<void> {
  let offset = 0;
...
export async function triggerCompanyScrape(
  env: CompanyScrapeEnv,
  companyId?: string,
  options: ScrapeOptions = {}
  ...
  if (companyId) {
    ...
    await scrapeCompany(env, company as CompanyRow, {
      ...options,
      adminTriggered: true,
    });
    return { processed: 1 };
