...
export class ApplicantStorageService {
  ...
  private safeParse<T>(
    ...
    const rawObject: Record<string, any> = { ...row };
  ...
  async getApplicantProfile(userId: string): Promise<ApplicantProfile | null> {
    ...
    return this.safeParse(result, ApplicantProfileSchema, [
      "target_roles",
      "skills",
      "preferences",
    ]);
  ...
  async getJobHistorySubmissions(
    ...
    return results.map((row) =>
      this.safeParse(row, JobHistorySubmissionSchema)
    );
  }
  ...
  async getJobHistoryByApplicant(
    ...
    return results.map((row) =>
      this.safeParse(row, JobHistoryEntrySchema, [
        "skills_used",
        "technologies",
        "keywords",
      ])
   ...
   */
  async getJobRatingsByApplicant(
    applicantId: string
  ): Promise<
    (JobRating & { job_title: string; job_company: string; job_url: string })[]
  > {
    const { results } = await this.env.DB.prepare(
      `
    ...
    return results.map((row) =>
      this.safeParse(row, JobRatingWithJobInfoSchema, ["strengths", "gaps"])
    );
  }
