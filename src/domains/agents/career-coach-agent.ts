...
export class CareerCoachAgent extends Agent<CareerCoachAgentEnv> {
  ...
  private async getProfile(request: Request): Promise<Response> {
    try {
      ...
      // Get related data
      const [jobHistory, skills, careerGoals, industryInterests, salaryGoals] = await Promise.all([
        this.db.prepare("SELECT * FROM job_history WHERE profile_id = ? AND is_confirmed = 1").bind(profile.id).all(),
        this.db.prepare("SELECT * FROM skills WHERE profile_id = ? AND is_confirmed = 1").bind(profile.id).all(),
        this.db.prepare("SELECT * FROM career_goals WHERE profile_id = ? AND is_confirmed = 1").bind(profile.id).all(),
        this.db.prepare("SELECT * FROM industry_interests WHERE profile_id = ? AND is_confirmed = 1").bind(profile.id).all(),
        this.db.prepare("SELECT * FROM salary_goals WHERE profile_id = ? AND is_confirmed = 1").bind(profile.id).all()
      ]);
  ...
  private async updateProfile(request: Request): Promise<Response> {
    try {
      ...
      // Get current profile
      const currentProfile = await this.db.prepare(`
        SELECT * FROM applicant_profiles WHERE user_id = ? AND is_active = 1
      `).bind(user_id).first();
  ...
  private async deleteProfile(request: Request): Promise<Response> {
    try {
      ...
      await this.db.prepare(`
        ...
        WHERE id = ?
      `).bind(new Date().toISOString(), currentProfile.id).run();
      ...
      return new Response(JSON.stringify({
        message: "Profile deleted successfully (soft delete). Awaiting confirmation.",
        requires_approval: true
      }), {
        headers: { "Content-Type": "application/json" }
      });
  ...
  private async analyzeDocument(request: Request): Promise<Response> {
    try {
      ...
      const { document_type, content, analysis_type = "general", job_description } = data;
  ...
  private async approveChanges(request: Request): Promise<Response> {
    try {
      ...
      for (const changeId of change_ids) {
        // Get the change
        const change = await this.db.prepare(`
  ...
  private async chatWithAgent(request: Request): Promise<Response> {
    try {
      ...
      return new Response(JSON.stringify({
        response: response.message || "I'm here to help with your career profile!",
   ...
   */
  private async logProfileChange(profileId: string, change: ProfileChange): Promise<void> {
...
Format as JSON with keys: analysis, improvements, confidence, score.`;
  ...
  private buildInterviewPrepPrompt(
    ...
    return `Create ${prepType} interview preparation for ${companyName ? `a position at ${companyName}` : 'a general interview'}:
...
${currentProfile ? `Current Profile Data:
...
Format as JSON with keys: suggestions, improvements, score, next_steps.`;
   ...
   * Builds chat prompt
   */
  private buildChatPrompt(message: string, userId: string, context?: any): string {
