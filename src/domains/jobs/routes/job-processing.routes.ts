...
jobProcessing.post(
  "/submit",
  zValidator("json", JobProcessingSubmitSchema),
  async (c) => {
    try {
      const data = c.req.valid("json");

      console.log(
        `🚀 Submitting ${data.urls.length} job URLs from ${data.source}...`
      );
...
jobProcessing.get(
  "/status",
  zValidator("query", JobProcessingStatusSchema),
  async (c) => {
    try {
      const { source, status, limit } = c.req.valid("query");

      const service = new JobProcessingService(c.env);
      const entries = await service.getJobQueueEntries({
        source,
        status,
        limit,
      });
...
jobProcessing.get("/stats", async (c) => {
  try {
    const service = new JobProcessingService(c.env);
    const allEntries = await service.getJobQueueEntries({ limit: 1000 });

    const stats = {
      total: allEntries.length,
      pending: allEntries.filter((e: any) => e.status === "pending").length,
      processing: allEntries.filter((e: any) => e.status === "processing")
        .length,
      completed: allEntries.filter((e: any) => e.status === "completed").length,
      failed: allEntries.filter((e: any) => e.status === "failed").length,
      by_source: allEntries.reduce(
