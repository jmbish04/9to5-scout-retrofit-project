...
export class JobMonitor extends DurableObject {
  ...
  private async monitorJob(req: Request): Promise<Response> {
    ...
    await this.ctx.storage.put("job_id", job_id);
    await this.ctx.storage.put("job_url", url);
    await this.ctx.storage.put("check_interval_hours", check_interval_hours);
    await this.ctx.storage.put("last_check", new Date().toISOString());
    await this.ctx.storage.put("status", "monitoring");
    ...
    await this.storageService.updateJobMonitoringStatus(
      job_id,
      "monitoring",
      nextCheck
    );
  ...
  private async checkJob(): Promise<Response> {
    const jobUrl = (await this.ctx.storage.get("job_url")) as string;
    const jobId = (await this.ctx.storage.get("job_id")) as string;
    ...
    await this.ctx.storage.put("last_check", result.last_check);
    await this.ctx.storage.put("status", result.status);
  ...
  private async getStatus(): Promise<Response> {
    const jobId = await this.ctx.storage.get("job_id");
    const status = (await this.ctx.storage.get("status")) || "idle";
    const lastCheck = await this.ctx.storage.get("last_check");
    const checkInterval =
      (await this.ctx.storage.get("check_interval_hours")) || 24;
  ...
  async alarm(): Promise<void> {
    try {
      ...
      const finalStatus = await this.ctx.storage.get("status");
      if (finalStatus === "monitoring" || finalStatus === "job_active") {
        const checkInterval =
          ((await this.ctx.storage.get("check_interval_hours")) as number) ||
          24;
