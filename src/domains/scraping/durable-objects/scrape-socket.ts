...
export class ScrapeSocket {
  ...
  private sendToClients(
    predicate: (info: ClientInfo) => boolean,
    payload: string,
    exclude?: WebSocket
  ): boolean {
    let delivered = false;
    for (const [socket, info] of this.clients.entries()) {
      ...
      try {
        ...
        console.warn(
          "Failed to send message to client, removing from registry:",
          error
        );
  ...
  private dispatchCommand(
    ...
    const delivered = this.sendToClients(
  ...
  private handleObserverMessage(
    ...
    // Handle job processing messages directly
    if (
      parsed &&
      typeof parsed === "object" &&
      (parsed as any).type === "job-processing"
    ) {
      this.handleJobProcessingMessage(socket, parsed as any);
      return;
    }
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (url.pathname === "/dispatch" && req.method === "POST") {
      ...
      const dispatched = this.sendToClients(
        (info) => isPythonClient(info),
        message
      );
      return new Response(dispatched ? "sent" : "no-python-clients", {
        status: dispatched ? 200 : 503,
      });
    }
