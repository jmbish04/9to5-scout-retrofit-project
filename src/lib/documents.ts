...
import { buildR2Url } from './email';
import { computeEmbedding, shouldReindex, upsertVector, stripMarkdown } from './vectorize';
...
export function serialiseEditorJson(editor: unknown): string {
  ...
  try {
    ...
    const visit = (node: any, depth: number = 0) => {
      ...
      if (type === 'paragraph') {
        const text = Array.isArray(content)
          ? content.map((child: any) => (typeof child?.text === 'string' ? child.text : '')).join('')
          : '';
        nodes.push(text);
        nodes.push('');
        return;
    ...
    return markdown.trim().length > 0 ? markdown.trim() : JSON.stringify(editor, null, 2);
  } catch (error) {
    console.warn('Failed to serialise editor JSON; falling back to JSON string.', error);
...
async function storeAsset(
  ...
  return { key, url: buildR2Url(env, key) };
}
...
async function storeDocumentAssets(
  ...
  const pdfResult = await storeAsset(env, pdfKey, pdfContent, 'application/pdf');

  let editorResult: { key: string; url: string | null } | null = null;
  if (editorKey) {
    const editorString = typeof editorJson === 'string' ? editorJson : JSON.stringify(editorJson);
    editorResult = await storeAsset(env, editorKey, editorString, 'application/json');
  }
...
async function readR2Text(env: Env, key: string | null | undefined): Promise<string> {
  if (!key) {
    return '';
  }
  const bucket = ensureBucket(env);
...
async function loadDocumentRecord(env: Env, id: number): Promise<ApplicantDocumentWithSections | null> {
  ...
  ).bind(id).first<ApplicantDocument>();
  ...
  ).bind(id).first<ResumeSections>();

  const editorUrl = buildR2Url(env, `documents/${id}/editor.json`);
...
export function buildResumeSectionInsert(documentId: number, sections?: ResumeSections | null) {
  if (!sections) {
    return null;
  }
...
async function upsertResumeSections(env: Env, documentId: number, docType: DocumentType, sections?: ResumeSections | null) {
  ...
  if (payload) {
    await env.DB.prepare(payload.sql).bind(...payload.binds).run();
  }
}
...
async function recordEmbedding(
  ...
  ).bind(documentId, vectorId).first<{ content_sha256: string }>();

  const should = shouldReindex(existing?.content_sha256 || null, embedding.hash);
...
async function indexDocument(env: Env, document: ApplicantDocumentWithSections, markdown: string): Promise<boolean> {
  let reindexed = false;
  const baseText = markdown || '';

  if (baseText.trim().length > 0) {
    const changed = await recordEmbedding(env, document.id, document, 'full', baseText);
  ...
  if (document.doc_type === 'resume' && document.resume_sections) {
    ...
    for (const [key, value] of pairs) {
      if (value && stripMarkdown(value).length > 0) {
        const changed = await recordEmbedding(env, document.id, document, key, value);
        reindexed = reindexed || changed;
      }
    }
  }
...
export async function createApplicantDocument(env: Env, input: DocumentCreateInput): Promise<ApplicantDocumentWithSections> {
  if (!input.user_id) {
  ...
  const markdown = (input.content_md && input.content_md.trim().length > 0)
  ...
  const insertResult = await env.DB.prepare(
    ...
    .bind(input.user_id, input.job_id || null, input.doc_type, input.purpose || null, input.title || null)
    .run();

  const documentId = Number(insertResult.meta.last_row_id);

  const assets = await storeDocumentAssets(env, documentId, markdown, input.editor_json);

  await env.DB.prepare(
    'UPDATE applicant_documents SET r2_key_md = ?1, r2_url_md = ?2, r2_key_pdf = ?3, r2_url_pdf = ?4, updated_at = ?5 WHERE id = ?6'
  )
    .bind(assets.mdKey, assets.mdUrl, assets.pdfKey, assets.pdfUrl, nowIso(), documentId)
    .run();

  await upsertResumeSections(env, documentId, input.doc_type, input.sections || null);
  ...
  return {
    ...document,
    resume_sections: input.doc_type === 'resume' ? (document.resume_sections || input.sections || undefined) : undefined,
    editor_json_url: assets.editorUrl || document.editor_json_url || null,
  };
}

export async function getApplicantDocument(env: Env, id: number): Promise<ApplicantDocumentWithSections | null> {
...
export async function updateApplicantDocument(
...
): Promise<{ document: ApplicantDocumentWithSections; reindexed: boolean } | null> {
  const existing = await loadDocumentRecord(env, id);
  if (!existing) {
    return null;
  }

  console.debug('documents:update', { id, hasContent: Boolean(input.content_md), hasSections: Boolean(input.sections) });
  ...
  if (input.content_md || input.editor_json) {
    markdown = (input.content_md && input.content_md.trim().length > 0)
      ? input.content_md.trim()
      : serialiseEditorJson(input.editor_json);

    const assets = await storeDocumentAssets(env, id, markdown, input.editor_json);
  ...
  await upsertResumeSections(env, id, existing.doc_type, input.sections || existing.resume_sections || undefined);
  ...
  const markdownContent = markdown !== null ? markdown : await readR2Text(env, document.r2_key_md);
...
export async function deleteApplicantDocument(env: Env, id: number): Promise<void> {
  ...
  ).bind(id).all<{ vectorize_id: string | null }>();

  const ids = (embeddings.results || [])
    .map((row) => row.vectorize_id)
  ...
  await env.DB.prepare('DELETE FROM applicant_documents WHERE id = ?1').bind(id).run();
}

export async function searchApplicantDocuments(env: Env, params: VectorSearchRequest): Promise<DocumentSearchResponse> {
  ...
  const matches: DocumentSearchMatch[] = (result.matches || []).map((match: any) => ({
    id: match.id,
    score: match.score,
...
export function scoreKeywords(resume: string, job: string): number {
  const resumeWords = new Set(stripMarkdown(resume).toLowerCase().split(/[^a-z0-9%]+/g).filter(Boolean));
  const jobWords = new Set(stripMarkdown(job).toLowerCase().split(/[^a-z0-9%]+/g).filter(Boolean));
  if (jobWords.size === 0) {
    return 50;
  }
  let matchCount = 0;
...
export function scoreStructure(resume: string): number {
  ...
  const hits = requiredSections.filter((section) => lower.includes(section)).length;
...
export function scoreSeniority(resume: string, job: string): number {
  ...
  const resumeScore = seniorityKeywords.filter((word) => resumeText.includes(word)).length;
  const jobScore = seniorityKeywords.filter((word) => jobText.includes(word)).length;
  if (jobScore === 0) {
...
export function buildRecommendation(line: number, target: string, suggestion: string): AtsRecommendation {
  return {
    target,
...
async function fetchJobMarkdown(env: Env, jobId: string): Promise<string> {
  ...
  ).bind(jobId).first<{ description_md?: string; requirements_md?: string; title?: string; company?: string }>();
  if (!job) {
    throw new Error('Job not found');
  }
  const parts = [job.title && `# ${job.title}`, job.company && `**Company:** ${job.company}`, job.description_md, job.requirements_md]
    .filter((value): value is string => Boolean(value));
  return parts.join('\n\n');
}

export async function evaluateDocumentAgainstJob(env: Env, documentId: number, jobId: string): Promise<AtsEvaluation> {
  ...
  const overall = clampScore(
    (keywordsScore + actionVerbScore + impactScore + brevityScore + structureScore + seniorityScore) / 6,
  );
  ...
  lines.forEach((line, index) => {
    ...
    if (!/[0-9%]/.test(line) && /managed|led|responsible/i.test(line)) {
      ...
      recommendations.push(buildRecommendation(index + 1, 'experience', suggestion));
    }
  });
...
async function buildUserProfileSummary(env: Env, userId: string): Promise<string> {
  const profile = await env.DB.prepare(
    'SELECT name, current_title, target_roles, skills FROM applicant_profiles WHERE user_id = ?1'
  ).bind(userId).first<{ name?: string; current_title?: string; target_roles?: string; skills?: string }>();
...
async function callDocumentGenerator(env: Env, prompt: string): Promise<string> {
...
export async function generateDocumentForJob(env: Env, input: DocumentGenerationInput): Promise<ApplicantDocumentWithSections> {
  ...
  console.debug('documents:generate', { user_id: input.user_id, job_id: input.job_id, doc_type: input.doc_type });
...
function comparePositions(a: PatchRangePosition, b: PatchRangePosition): number {
  if (a.line === b.line) {
    return a.col - b.col;
  }
...
function offsetFromPosition(text: string, position: PatchRangePosition): number {
...
function applyPatchSequence(content: string, patches: DocumentPatch[]): { text: string; summary: string[] } {
  const sorted = [...patches].sort((a, b) => comparePositions(b.range.start, a.range.start));
  let text = content;
...
export async function applyDocumentPatches(env: Env, id: number, patches: DocumentPatch[]): Promise<ApplyPatchResult> {
  const document = await loadDocumentRecord(env, id);
  if (!document) {
    throw new Error('Document not found');
  }
  ...
  await env.DB.prepare('UPDATE applicant_documents SET updated_at = ?1 WHERE id = ?2')
    .bind(nowIso(), id)
    .run();
