...
export class TestStreamingService {
  ...
  private testResult: TestStreamResult;

  constructor(ws: WebSocket, env: Env, testId: string) {
    this.ws = ws;
    this.env = env;
    this.testId = testId;
    this.testResult = {
  ...
  public async startTest(
    testName: string,
    steps: Omit<TestStep, "status" | "startTime" | "endTime" | "duration">[]
  ): Promise<void> {
   ...
   */
  public sendResult(data: any): void {
   ...
   * Complete the test
   */
  public completeTest(success: boolean, finalResults?: any): void {
    const now = new Date().toISOString();
   ...
   */
  private sendMessage(type: TestStreamMessage["type"], data: any): void {
   ...
   */
  public getTestResult(): TestStreamResult {
 ...
 */
export async function handleTestStreamingWebSocket(
  request: Request,
  env: Env
): Promise<Response> {
  ...
  if (url.pathname === "/api/test-streaming/ws") {
    ...
    if (server) {
      ...
      server.addEventListener("error", (error) => {
        console.error(
          `Test streaming WebSocket error for test ${testId}:`,
          error
        );
...
async function handleTestMessage(
  message: any,
  streamingService: TestStreamingService,
  env: Env
): Promise<void> {
  switch (message.type) {
