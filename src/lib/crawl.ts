...
export async function crawlJobWithSnapshot(
  ...
  try {
    ...
    const job = await extractJob(env as any, html, url, siteId || "unknown");
...
export async function crawlJob(
  ...
  try {
    ...
    const response = await browser2.fetch(
      "https://browser.render.cloudflare.com",
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (jobTitle && companyName) {
        console.log(
          `Attempting fallback search with Google Talent API for "${jobTitle}" at "${companyName}"`
        );
        const talentResponse = await searchJobs(
          env as any,
          jobTitle,
          companyName
        );
        if (
          talentResponse &&
          talentResponse.results &&
          talentResponse.results.length > 0
        ) {
          console.log("Successfully found job via Google Talent API fallback.");
          const talentJob = talentResponse.results[0]; // Get the first result
          if (!talentJob) return null;
          // Ensure url is always a string for Job interface compatibility
          const jobWithUrl = { ...talentJob, url: talentJob.url || "" };
          const jobId = await saveJob(env, jobWithUrl);
          return { ...jobWithUrl, id: jobId };
        }
      }
      return null;
    }
