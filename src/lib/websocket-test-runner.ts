...
import { TestStreamingService } from "./test-streaming";
...
export class WebSocketTestRunner {
   ...
   */
  private async runTestType(testType: string, env: Env): Promise<any> {
    switch (testType) {
      case "unit":
   ...
   */
  private async runTalentTests(env: Env): Promise<any> {
   ...
   * Run browser tests
   */
  private async runBrowserTests(env: Env): Promise<any> {
   ...
   */
  private async runE2ETests(env: Env): Promise<any> {
  ...
  private generateTestSteps(
    testTypes: string[]
  ): Array<{ step: number; name: string }> {
    const steps: Array<{ step: number; name: string }> = [];
    let stepNumber = 1;
  ...
  private generateFinalReport(results: Record<string, any>): any {
    ...
    const totalDuration = Object.values(results).reduce(
      (sum, result) => sum + parseFloat(result.duration),
      0
    );
 ...
 */
export function createWebSocketTestRunner(
  testId: string,
  testTypes: ("unit" | "integration" | "talent" | "browser" | "e2e")[],
  streamingService: TestStreamingService,
  env: Env
