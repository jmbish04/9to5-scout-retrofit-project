...
export class D1ValidationError extends Error implements D1Error {
...
export class D1ConstraintError extends Error implements D1Error {
...
export async function insertJob(
  ...
  try {
    ...
    const result = await db
      ...
      .bind(
        validatedJob.id,
        validatedJob.site_id,
        validatedJob.url,
        validatedJob.canonical_url,
        validatedJob.title,
        validatedJob.company,
        validatedJob.location,
        validatedJob.employment_type,
        validatedJob.department,
        validatedJob.salary_min,
        validatedJob.salary_max,
        validatedJob.salary_currency,
        validatedJob.salary_raw,
        validatedJob.compensation_raw,
        validatedJob.description_md,
        validatedJob.requirements_md,
        validatedJob.posted_at,
        validatedJob.closed_at,
        validatedJob.status,
        validatedJob.last_seen_open_at,
        validatedJob.first_seen_at,
        validatedJob.last_crawled_at,
        validatedJob.daily_monitoring_enabled ? 1 : 0,
        validatedJob.monitoring_frequency_hours,
        validatedJob.last_status_check_at,
        validatedJob.closure_detected_at
      )
      .run();
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new D1ValidationError(
        "Job update data validation failed",
        error.issues
      );
    }
  ...
  try {
    ...
    const result = await db
      ...
      .bind(
        ...
        validatedSnapshot.job_id,
        validatedSnapshot.run_id,
        validatedSnapshot.content_hash,
        validatedSnapshot.html_r2_key,
        validatedSnapshot.json_r2_key,
        validatedSnapshot.screenshot_r2_key,
        validatedSnapshot.pdf_r2_key,
        validatedSnapshot.markdown_r2_key,
        validatedSnapshot.fetched_at,
        validatedSnapshot.http_status,
        validatedSnapshot.etag
      )
      .run();
 ...
 */
export async function insertJobTrackingHistory(
  db: D1Database,
  ...
  try {
    ...
    const result = await db
      ...
      .bind(
        validatedTracking.id,
        validatedTracking.job_id,
        validatedTracking.snapshot_id,
        validatedTracking.tracking_date,
        validatedTracking.status,
        validatedTracking.content_hash,
        validatedTracking.title_changed ? 1 : 0,
        validatedTracking.requirements_changed ? 1 : 0,
        validatedTracking.salary_changed ? 1 : 0,
        validatedTracking.description_changed ? 1 : 0,
        validatedTracking.error_message,
        validatedTracking.created_at
      )
      .run();
    ...
    if (error instanceof z.ZodError) {
      throw new D1ValidationError(
        "Tracking data validation failed",
        error.issues
      );
    }
