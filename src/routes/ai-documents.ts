...
export async function handleCoverLetterPost(request: Request, env: Env): Promise<Response> {
  ...
  const coverLetterSchema = {
    ...
    properties: {
      ...
      body_paragraph_2: {
        type: 'string',
        description: 'Second body paragraph highlighting company fit and motivation',
      },
      closing_paragraph: {
        type: 'string',
  ...
  const response = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
    messages: [
      {
        role: 'system',
        content: 'You are an expert career coach and copywriter. Generate structured cover letters that are polished, professional, and tailored to the job description provided.',
      },
      {
        role: 'user',
        content: prompt,
  ...
  const structured = JSON.parse(response.response || '{}') as CoverLetterContent;
...
export async function handleResumePost(request: Request, env: Env): Promise<Response> {
  ...
  const response = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
    messages: [
      {
        role: 'system',
        content: 'You are an expert resume writer. Return structured resume content tailored to the provided job description.',
      },
      {
        role: 'user',
        content: prompt,
