...
function normalizeRawJson(value: unknown): Record<string, any> | null {
  ...
  if (typeof value === "string" && value.trim().length > 0) {
    try {
      ...
      console.warn(
        "Failed to parse raw JSON payload from intake submission:",
        error
      );
      return null;
    }
...
function normalizeIntakePayload(
  raw: any
): NormalizedIntakePayload | { error: string } {
  ...
  const metadataObject = normalizeRawJson(
    (raw as any).metadata ?? (raw as any).details ?? (raw as any).job
  );
...
async function enqueueIntakeSubmission(
  env: Env,
...
async function processIntakeQueue(env: any, maxToProcess = INTAKE_MAX_BATCH) {
  ...
  do {
    ...
    claimed = await claimIntakeJobs(
      env,
      Math.min(remainingCapacity, INTAKE_CLAIM_SIZE)
    );
    ...
    for (const row of claimed) {
      ...
      if (!payload || typeof payload !== "object") {
        await markIntakeFailed(
          ...
          rowId,
          attempts,
          "Invalid payload in queue entry."
        );
        continue;
      }
      ...
      const metadata =
        payload.metadata && typeof payload.metadata === "object"
          ...
          : null;
      const rawJson =
        payload.rawJson && typeof payload.rawJson === "object"
          ? payload.rawJson
          : null;
      ...
      try {
        ...
        await markIntakeFailed(
          ...
          rowId,
          attempts,
          error?.message || "Failed to process intake item."
        );
      }
    }
  ...
  try {
    ...
    if (urls.length === 0) {
      return errorResponse(
        'At least one target URL must be provided in the "urls" field.'
      );
    }
    ...
    const explicitPayload =
      "payload" in rawPayload ? rawPayload.payload : undefined;
    ...
    const payloadSource =
      explicitPayload ??
      metadata ??
      (Object.keys(rawPayload).length > 0 ? rawPayload : null);
    ...
    const availableAtRaw =
      (body as Record<string, unknown>).available_at ??
      (body as Record<string, unknown>).availableAt;
    ...
    if (availableAtRaw) {
      ...
      if (Number.isNaN(availableDate.getTime())) {
        return errorResponse(
          "The provided available_at value is not a valid date."
        );
      }
    ...
    return jsonResponse(
      { queue: record ? transformQueueRow(record) : null },
      201
    );
  } catch (error) {
...
export async function handleScrapeQueueUnrecordedGet(
  request: Request,
  env: any
): Promise<Response> {
  try {
...
export async function handleScraperIntakePost(
  request: Request,
  env: any
): Promise<Response> {
  try {
    ...
    for (const [index, submission] of submissions.entries()) {
      ...
      try {
        ...
        failed.push({
          index,
          error: error?.message || "Failed to enqueue submission.",
        });
      }
    }
    ...
    if (queued.length === 0) {
      ...
      return errorResponse(
        firstFailed ? firstFailed.error : "No submissions were accepted."
      );
    }
...
export async function handleScrapedJobDetailsPost(
  request: Request,
  env: any
): Promise<Response> {
  try {
    ...
    const jobUrl =
      typeof data.job_url === "string" && data.job_url.length > 0
        ? data.job_url
    ...
    if (metadataValue && typeof metadataValue === "string") {
      try {
        ...
        console.warn(
          "Failed to parse metadata JSON string in scraped job details:",
          error
        );
      }
    ...
    const monitoredJobIdRaw =
      data.monitored_job_id ?? (data as any).monitoredJobId;
    const monitoredJobId =
      typeof monitoredJobIdRaw === "string" &&
      monitoredJobIdRaw.trim().length > 0
    ...
    const insertResult = await env.DB.prepare(
      ...
      .bind(
        ...
        typeof data.employment_type === "string"
          ...
          : typeof (data as any).employmentType === "string"
          ? (data as any).employmentType
          : null,
        typeof data.salary === "string" ? data.salary : null,
        typeof data.apply_url === "string"
          ? data.apply_url
          : typeof (data as any).applyUrl === "string"
          ? (data as any).applyUrl
          : null,
    ...
    await processJobIngestion(env, {
      ...
      companyName:
        typeof data.company === "string"
          ? data.company
          : metadataObject?.company?.name,
      companyWebsite:
        typeof (data as any).company_url === "string"
          ? (data as any).company_url
          : metadataObject?.company_url,
      html: typeof (data as any).html === "string" ? (data as any).html : null,
      description:
        typeof data.description === "string" ? data.description : null,
      text: typeof data.description === "string" ? data.description : undefined,
      metadata: metadataObject,
      dryRun,
    });

    return jsonResponse(
      { job: record ? transformJobDetailsRow(record) : null },
      201
    );
  } catch (error) {
...
export async function handleScraperMonitoredJobsGet(
  request: Request,
  env: any
): Promise<Response> {
  try {
