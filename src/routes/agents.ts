...
export async function handleAgentsGet(request: Request, env: any): Promise<Response> {
  try {
...
export async function handleAgentsPost(request: Request, env: any): Promise<Response> {
  try {
    const agent: AgentConfig = await request.json();
    
    // Validate required fields
    if (!agent.id || !agent.name || !agent.role || !agent.goal || !agent.backstory || !agent.llm) {
...
export async function handleAgentGet(request: Request, env: any, agentId: string): Promise<Response> {
  try {
...
export async function handleAgentPut(request: Request, env: any, agentId: string): Promise<Response> {
  try {
...
export async function handleAgentDelete(request: Request, env: any, agentId: string): Promise<Response> {
  try {
    // Check if agent exists
    const { results } = await env.DB.prepare(
      'SELECT id FROM agent_configs WHERE id = ?'
    ...
    await env.DB.prepare('DELETE FROM agent_configs WHERE id = ?').bind(agentId).run();
