...
export async function handleAgentQuery(request: Request, env: any): Promise<Response> {
  try {
    ...
    const jobIds = searchResults.matches?.map((match: any) => match.id) || [];
    ...
    const jobsWithScores = jobs.map((job: any) => {
      const match = searchResults.matches?.find((m: any) => m.id === job.id);
      return {
        ...job,
    ...
    jobsWithScores.sort((a: any, b: any) => b.similarity_score - a.similarity_score);
