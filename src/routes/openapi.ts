 ...
 */
export async function handleOpenAPIDocs(
  request: Request,
  env: Env
): Promise<Response> {
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to generate OpenAPI documentation" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
 ...
 * Generate Swagger UI HTML
 */
function generateSwaggerUI(openApiUrl: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
