...
import { getJobTrackingHistory, getJobsForMonitoring } from '../lib/storage';
import { getJobTrackingTimeline, runDailyJobMonitoring } from '../lib/monitoring';

/**
 * GET /api/jobs/:jobId/tracking
 * Get tracking history for a specific job.
 */
export async function handleJobTrackingGet(request: Request, env: any): Promise<Response> {
  try {
 ...
 */
export async function handleSnapshotContentGet(request: Request, env: any): Promise<Response> {
  try {
    const url = new URL(request.url);
    const pathParts = url.pathname.split('/');
    ...
    if (!jobId || !snapshotId) {
      return new Response(JSON.stringify({ error: 'Job ID and snapshot ID are required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    switch (contentType) {
      case 'html':
        r2Key = snapshot.html_r2_key;
        mimeType = 'text/html';
        break;
      case 'pdf':
        r2Key = snapshot.pdf_r2_key;
        mimeType = 'application/pdf';
        break;
      case 'markdown':
        r2Key = snapshot.markdown_r2_key;
        mimeType = 'text/markdown';
        break;
      case 'json':
        r2Key = snapshot.json_r2_key;
        mimeType = 'application/json';
        break;
      case 'screenshot':
        r2Key = snapshot.screenshot_r2_key;
        mimeType = 'image/png';
        break;
      default:
    ...
    if (!r2Key) {
      return new Response(JSON.stringify({ error: `${contentType} content not available for this snapshot` }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!object) {
      return new Response(JSON.stringify({ error: 'Content not found in storage' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(content, {
      headers: { 
        ...
        'Content-Disposition': `inline; filename="${snapshotId}.${contentType === 'screenshot' ? 'png' : contentType}"`,
      },
    });
 ...
 */
export async function handleDailyMonitoringPost(request: Request, env: any): Promise<Response> {
  try {
 ...
 */
export async function handleMonitoringStatusGet(request: Request, env: any): Promise<Response> {
  try {
 ...
 */
export async function handleMonitoringQueueGet(request: Request, env: any): Promise<Response> {
  try {
    const url = new URL(request.url);
 ...
 */
export async function handleJobMonitoringPut(request: Request, env: any): Promise<Response> {
  try {
    const url = new URL(request.url);
    const pathParts = url.pathname.split('/');
    ...
    // Validate input
    if (daily_monitoring_enabled !== undefined && typeof daily_monitoring_enabled !== 'boolean') {
      return new Response(JSON.stringify({ error: 'daily_monitoring_enabled must be a boolean' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    if (monitoring_frequency_hours !== undefined && (typeof monitoring_frequency_hours !== 'number' || monitoring_frequency_hours < 1)) {
      return new Response(JSON.stringify({ error: 'monitoring_frequency_hours must be a positive number' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    // Get updated job
    const updatedJob = await env.DB.prepare('SELECT * FROM jobs WHERE id = ?').bind(jobId).first();
