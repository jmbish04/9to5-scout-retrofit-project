...
export async function handleTasksGet(
  ...
  try {
    ...
    const tasks = results.map((row: Record<string, unknown>) => ({
      ...row,
      context_tasks: row.context_tasks
        ? JSON.parse(row.context_tasks as string)
        : null,
...
export async function handleTasksPost(
  ...
  try {
    ...
    if (agentResults.length === 0) {
      return new Response(
        JSON.stringify({ error: "Referenced agent does not exist" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    if (task.context_tasks && task.context_tasks.length > 0) {
      const { results: contextResults } = await env.DB.prepare(
        `SELECT id FROM task_configs WHERE id IN (${task.context_tasks
          .map(() => "?")
          .join(",")})`
      )
      ...
      if (contextResults.length !== task.context_tasks.length) {
        return new Response(
          JSON.stringify({ error: "One or more context tasks do not exist" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          }
  ...
  try {
    ...
    const formattedTask = {
      ...
      context_tasks: task.context_tasks
        ? JSON.parse(task.context_tasks as string)
        : null,
      output_schema: task.output_schema
        ? JSON.parse(task.output_schema as string)
        : null,
...
export async function handleTaskPut(
  request: Request,
  env: Env,
  taskId: string
): Promise<Response> {
  try {
    ...
    const formattedTask = {
      ...
      context_tasks: updatedTask.context_tasks
        ? JSON.parse(updatedTask.context_tasks as string)
        : null,
      output_schema: updatedTask.output_schema
        ? JSON.parse(updatedTask.output_schema as string)
        : null,
...
export async function handleTaskDelete(
  ...
  try {
    ...
    const countResult = workflowResults[0] as { count: number };
    ...
    const contextCountResult = contextResults[0] as { count: number };
    if (contextCountResult.count > 0) {
      return new Response(
        JSON.stringify({
          error:
            "Cannot delete task: it is referenced as context by other tasks",
        }),
        {
          status: 409,
          headers: { "Content-Type": "application/json" },
