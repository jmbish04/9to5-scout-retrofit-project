...
export interface ScrapeFallbackRequest {
  ...
  source?: string;
  priority?: number;
  reason?: string; // Why browser rendering failed
}

export async function handleScrapeFallbackPost(
  request: Request,
  env: Env
): Promise<Response> {
  try {
    const body = (await request.json()) as ScrapeFallbackRequest;
    ...
    if (!queueResponse.ok) {
      return new Response(
        JSON.stringify({ error: "Failed to queue job for Python processing" }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
        }
    ...
    const queueResult = (await queueResponse.json()) as { job: { id: number } };
    ...
    return new Response(
      JSON.stringify({ error: "Failed to queue fallback job" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
