...
type WorkflowBindingKey = keyof Pick<
  Env,
  "DISCOVERY_WORKFLOW" | "JOB_MONITOR_WORKFLOW" | "CHANGE_ANALYSIS_WORKFLOW"
>;
...
async function stageCustomWorkflowExecution(
  env: Env,
  workflow: any,
  taskSequence: string[],
  ...
  await env.DB.prepare(
    ...
    .bind("queued", JSON.stringify(stats), runId)
    .run();
...
export async function handleWorkflowExecute(
  ...
  try {
    ...
    const bindingOverride =
      typeof requestBody.binding === "string" ? requestBody.binding : null;
    ...
    const { runId, stats } = await stageCustomWorkflowExecution(
      env,
      workflow,
      taskSequence,
      context
    ...
    const responsePayload = {
      ...
      stats,
      message:
        "Workflow has been staged for custom orchestration. Monitor run records for progress updates.",
    };
